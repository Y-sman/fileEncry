...
from owl_common.domain.vo import ResetPwdBody
...
def system_get_user(id:Optional[int]=None):
    ...
    if id:
        ...
        if user is not None:
            # 序列化时 dept_id 须为 int 或 None，避免 DB 空串导致 ValidationError
            d = getattr(user, "dept_id", None)
            if d == "" or (d is not None and not isinstance(d, int)):
                user.dept_id = None
        setattr(ajax_response,"data",user)
        post_ids = SysPostService.select_post_list_by_user_id(id) if id else []
        setattr(ajax_response,"post_ids",post_ids)
        # 从 user.roles 提取 role_id，或按用户 ID 查询（mapper 只填充了 roles 列表，无 role_ids）
        role_ids = [r.role_id for r in (getattr(user, "roles", []) or []) if getattr(r, "role_id", None) is not None]
        if not role_ids and id:
            from owl_system.mapper.sys_role import SysRoleMapper
            role_ids = list(SysRoleMapper.select_role_list_by_user_id(id))
        # 只返回当前仍存在的角色 ID，已删除角色不再显示为已选
        valid_role_ids = {r.role_id for r in roles if getattr(r, "role_id", None) is not None}
        role_ids = [rid for rid in role_ids if rid in valid_role_ids]
        setattr(ajax_response,"role_ids",role_ids)
...
def system_create_user(dto:SysUser):
    ...
    # 未传或为空时从 sys_config（sys.user.initPassword）读取初始密码，与数据库配置一致
    if not (dto.password and str(dto.password).strip()):
        dto.password = (SysConfigService.select_config_by_key("sys.user.initPassword") or "123456").strip() or "123456"
    enc = SecurityUtil.encrypt_password(dto.password)
    dto.password = enc.decode("utf-8") if isinstance(enc, bytes) else enc
    if not dto.status:
        dto.status = "0"
...
def system_delete_users(
    ...
    '''
        删除用户
    '''
    # Path 参数可能以字符串 "100" 传入且未经过 validate_call，必须在此统一转为 List[int]
    try:
        ids = ids_to_list(ids) if not isinstance(ids, list) else (ids or [])
    except (ValueError, TypeError):
        return AjaxResponse.from_error("用户ID格式无效")
    if not ids:
        return AjaxResponse.from_error("请选择要删除的用户")
...
@Log(title="用户管理",business_type=BusinessType.UPDATE)
@JsonSerializer()
def system_update_user_resetpwd(dto: ResetPwdBody):
    '''
        重置密码（使用 ResetPwdBody 确保请求体中的 password 被正确接收）
    '''
    user_id = dto.userId
    SysUserService.check_user_allowed(SysUser(user_id=user_id))
    SysUserService.check_user_data_scope(user_id)
    if not (dto.password and dto.password.strip()):
        return AjaxResponse.from_error("新密码不能为空")
    enc = SecurityUtil.encrypt_password(dto.password.strip())
    pwd = enc.decode("utf-8") if isinstance(enc, bytes) else enc
    flag = SysUserService.reset_pwd_by_user_id(user_id, pwd)
    ajax_response = AjaxResponse.from_success() if flag else AjaxResponse.from_error()
    return ajax_response
...
def system_get_user_authrole(id:int):
    ...
    if sysuser is not None:
        d = getattr(sysuser, "dept_id", None)
        if d == "" or (d is not None and not isinstance(d, int)):
            sysuser.dept_id = None
    roles:List[SysRole] = SysRoleService.select_role_list_by_user_id(id)
    if sysuser and not sysuser.is_admin():
        roles = [role for role in roles if not role.is_admin()]
