...
body_valid_config = ConfigDict(
    from_attributes = False,
    alias_generator = to_camel,  
    frozen = True,
    extra = "ignore",  # 忽略额外字段，不报错也不添加到模型
    strict = True,
    populate_by_name = False,
)
...
class BaseSchemaFactory(AbcSchemaFactory):
    ...
    def rebuild_model(self, model_cls:Type[BaseEntity]) -> Type[BaseEntity]:
        ...
        # 创建新模型时使用自己的 model_config，而不是继承原模型的配置
        # 在 Pydantic v2 中，需要使用 __pydantic_config__ 来设置配置
        # 合并配置，优先使用 self.model_config 中的设置
        merged_config_dict = {}
        if hasattr(model_cls, 'model_config'):
            # 如果是 ConfigDict，需要正确合并
            if isinstance(model_cls.model_config, dict):
                merged_config_dict = dict(model_cls.model_config)
            elif hasattr(model_cls.model_config, '__dict__'):
                merged_config_dict = dict(model_cls.model_config.__dict__)
            else:
                merged_config_dict = dict(model_cls.model_config) if hasattr(model_cls.model_config, '__iter__') else {}
        
        # 保存原模型的 frozen 设置（如果存在）
        original_frozen = merged_config_dict.get("frozen", None)
        
        # 用 self.model_config 覆盖，确保 extra 等配置正确应用
        if isinstance(self.model_config, dict):
            merged_config_dict.update(self.model_config)
        elif hasattr(self.model_config, '__dict__'):
            merged_config_dict.update(self.model_config.__dict__)
        else:
            merged_config_dict.update(dict(self.model_config) if hasattr(self.model_config, '__iter__') else {})
        
        # 恢复原模型的 frozen 设置（如果原模型有设置，优先使用原模型的设置）
        if original_frozen is not None:
            merged_config_dict["frozen"] = original_frozen
        
        # 强制确保 extra 配置被应用（对于 BodySchemaFactory，必须使用 extra="ignore"）
        if self.action == "body" and isinstance(self.model_config, dict) and "extra" in self.model_config:
            merged_config_dict["extra"] = self.model_config["extra"]
        
        # 创建新的 ConfigDict
        merged_config = ConfigDict(**merged_config_dict)
        ...
        new_model = create_model(
            ...
            __pydantic_config__=merged_config,
            **field_definitions
        )
        # 确保新模型使用正确的配置
        if hasattr(new_model, 'model_config'):
            new_model.model_config = merged_config
        return new_model
...
class BodySchemaFactory(BaseSchemaFactory):
    ...
    action = "body"
    model_config = body_valid_config
...
class QuerySchemaFactory(BaseSchemaFactory):
    ...
    def validate_annotation(self, annotation: Type) -> Optional[Type[BaseEntity]]:
        ...
        if issubclass(bo_model, BaseEntity):
            updated_model = self.rebuild_model(model_cls=bo_model)
            # 注意：不要覆盖 self.model_config，查询验证应该使用 query_valid_config（extra="allow"）
            # 原模型的配置（extra="forbid"）不适合查询验证，因为查询参数可能包含额外的字段
