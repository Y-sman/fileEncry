...
from pydantic import BaseModel, BeforeValidator, Field, Strict, computed_field, model_validator, field_serializer
...
from owl_common.base.transformer import int_to_str, to_datetime, str_to_int, optional_int
...
class SysUser(AuditEntity):
    ...
    dept_id: Annotated[
        Optional[int],
        BeforeValidator(optional_int),  # 非必填：无效值一律转为 None，不抛错
        Field(default=None, description="部门ID，非必填"),
        VoField(query=True),
    ...
    @classmethod
    def _normalize_dept_id_input(cls, data):
        """在字段校验前将 dept_id 规范为 int 或 None，避免 DB 空串/float 等导致 ValidationError"""
        if isinstance(data, dict) and "dept_id" in data:
            v = data["dept_id"]
            if v is not None and v != "":
                if isinstance(v, int):
                    pass
                elif isinstance(v, float) and v == int(v):
                    data = {**data, "dept_id": int(v)}
                else:
                    try:
                        data = {**data, "dept_id": int(v)}
                    except (ValueError, TypeError):
                        data = {**data, "dept_id": None}
            elif v == "":
                data = {**data, "dept_id": None}
        return data

    @model_validator(mode="after")
    def normalize_dept_id(self):
        """校验后统一 dept_id 为 int 或 None"""
        v = getattr(self, "dept_id", None)
        if v is not None and not isinstance(v, int):
            object.__setattr__(self, "dept_id", None)
        return self

    @field_serializer("dept_id")
    def serialize_dept_id(self, v):
        """序列化时只输出 int 或 None，避免空串导致 ValidationError"""
...
class SysRole(AuditEntity):
    ...
    menu_ids: Annotated[
        Optional[List[int]],
        Field(default=[],vo=VoAccess(body=True))
    ]
...
# 重建模型以解析前向引用
SysUser.model_rebuild()
LoginUser.model_rebuild()
SysRole.model_rebuild()
SysDept.model_rebuild()
SysMenu.model_rebuild()
TreeSelect.model_rebuild()
