...
class FileUploadUtil:
    ...
    def upload(cls, file:FileStorage, base_path:str) -> str:
        ...
        # 如果 base_path 是相对路径，需要转换为绝对路径
        if not os.path.isabs(base_path):
            # 获取项目根目录（假设当前工作目录是项目根目录）
            project_root = os.getcwd()
            base_path = os.path.join(project_root, base_path)
        # 规范化 base_path（确保使用正确的路径分隔符）
        base_path = os.path.normpath(base_path)
        # 规范化 filename（将所有路径分隔符替换为系统分隔符）
        # filename 格式可能是 "20251116/xxx_xxx.jpg"，需要替换为系统分隔符
        filename_normalized = filename.replace("/", os.sep).replace("\\", os.sep)
        # 使用规范化后的 filename 拼接路径
        filepath = os.path.join(base_path, filename_normalized)
        # 再次规范化完整路径（处理可能的异常情况）
        filepath = os.path.normpath(filepath)
        file_parpath = os.path.dirname(filepath)
        if not os.path.exists(file_parpath):
            os.makedirs(file_parpath, exist_ok=True)
        file.save(filepath)
        # 返回URL路径（用于访问），格式：/profile/upload/日期/文件名
        # 需要从 filepath 中提取相对于 base_path 的路径
        # filepath: D:/owl/uploadPath/upload/20251116/文件名
        # base_path: D:/owl/uploadPath/upload
        # 返回: /profile/upload/20251116/文件名
        try:
            # 计算 filename 相对于 base_path 的相对路径
            relative_path = os.path.relpath(filepath, base_path)
            # 确定子目录名称（upload/download/avatar/import）
            # 通过比较 base_path 和各个路径来确定
            from owl_common.config import OWLConfig
            config = OWLConfig()
            base_path_norm = os.path.normpath(base_path)
            if base_path_norm == os.path.normpath(config.upload_path):
                sub_dir = "upload"
            elif base_path_norm == os.path.normpath(config.download_path):
                sub_dir = "download"
            elif base_path_norm == os.path.normpath(config.avatar_path):
                sub_dir = "avatar"
            elif base_path_norm == os.path.normpath(config.import_path):
                sub_dir = "import"
            else:
                # 默认使用 upload
                sub_dir = "upload"
            # 构建URL路径
            resource_path = Constants.RESOURCE_PREFIX + "/" + sub_dir + "/" + relative_path.replace("\\", "/")
        except (ValueError, AttributeError):
            # 如果计算相对路径失败，使用简单的路径拼接
            # 从 filepath 中提取最后部分（日期/文件名）
            filename_parts = filename_normalized.split(os.sep)
            sub_dir = "upload"  # 默认
            if len(filename_parts) > 1:
                resource_path = Constants.RESOURCE_PREFIX + "/" + sub_dir + "/" + "/".join(filename_parts).replace("\\", "/")
            else:
                resource_path = Constants.RESOURCE_PREFIX + "/" + sub_dir + "/" + filename_normalized.replace("\\", "/")
