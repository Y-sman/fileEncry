    ...
    def init(cls):
        ...
        for job in SysJobMapper.select_job_all():
            try:
                ...
                failed_jobs.append((job.job_name, job.invoke_target, str(e)))
                logger.warning(
                    f"跳过无效的定时任务 [{job.job_name}] (ID: {job.job_id}): "
                    f"调用目标 '{job.invoke_target}' - {e}"
                )
        
        if failed_jobs:
            logger.warning(
                f"共有 {len(failed_jobs)} 个定时任务初始化失败，请检查表SysJob的数据。"
                f"失败的任务: {', '.join([name for name, _, _ in failed_jobs])}"
            )
