    ...
    def white_list_check(cls, invoke_target:str) -> bool:
        ...
        try:
            ...
            try:
                ...
                if whitelist_config:
                    # 配置值格式：逗号分隔的模块名，如 "owl_apscheduler,another_module"
                    whitelist_strs = [s.strip() for s in whitelist_config.split(",") if s.strip()]
                    for pack_name in whitelist_strs:
            ...
            # 如果解析失败（比如传入的不是有效的方法调用字符串），返回 False
            # 这样可以避免在验证阶段因为格式错误而抛出异常
            return False
