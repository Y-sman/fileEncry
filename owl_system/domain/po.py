...
from sqlalchemy import CHAR, DateTime, Index, String, Text, text
...
class SysFriendRequestPo(db.Model):
    __tablename__ = 'sys_friend_request'
    __table_args__ = {'comment': '好友申请表'}

    id: Mapped[int] = mapped_column(BIGINT(20), primary_key=True, autoincrement=True, comment='主键ID')
    from_user_id: Mapped[int] = mapped_column(BIGINT(20), comment='申请人ID')
    to_user_id: Mapped[int] = mapped_column(BIGINT(20), comment='被申请人ID')
    message: Mapped[Optional[str]] = mapped_column(String(500), comment='申请留言')
    status: Mapped[int] = mapped_column(TINYINT(4), server_default=text("'0'"), comment='状态: 0-待处理 1-同意 2-拒绝')
    create_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text("CURRENT_TIMESTAMP"), comment='创建时间')
    handle_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, comment='处理时间')


class SysUserFriendPo(db.Model):
    __tablename__ = 'sys_user_friend'
    __table_args__ = {'comment': '好友关系表'}

    id: Mapped[int] = mapped_column(BIGINT(20), primary_key=True, autoincrement=True, comment='主键ID')
    user_id: Mapped[int] = mapped_column(BIGINT(20), comment='用户ID')
    friend_id: Mapped[int] = mapped_column(BIGINT(20), comment='好友ID')
    status: Mapped[int] = mapped_column(TINYINT(4), server_default=text("'1'"), comment='状态: 1-已添加 2-已删除')
    create_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text("CURRENT_TIMESTAMP"), comment='创建时间')
    update_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, comment='更新时间')


class ChatMessagePo(db.Model):
    __tablename__ = 'chat_message'
    __table_args__ = {'comment': '聊天消息表'}

    id: Mapped[int] = mapped_column(BIGINT(20), primary_key=True, autoincrement=True, comment='消息ID')
    sender_id: Mapped[int] = mapped_column(BIGINT(20), comment='发送者ID')
    receiver_id: Mapped[int] = mapped_column(BIGINT(20), comment='接收者ID')
    msg_type: Mapped[int] = mapped_column(INTEGER(11), server_default=text("'1'"), comment='消息类型：1-文本，2-文件')
    content: Mapped[Optional[str]] = mapped_column(Text, comment='消息内容')
    file_id: Mapped[Optional[int]] = mapped_column(BIGINT(20), comment='文件ID')
    is_read: Mapped[int] = mapped_column(TINYINT(4), server_default=text("'0'"), comment='是否已读：0-未读，1-已读')
    create_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text("CURRENT_TIMESTAMP"), comment='创建时间')


class ChatSessionPo(db.Model):
    __tablename__ = 'chat_session'
    __table_args__ = {'comment': '聊天会话表'}

    id: Mapped[int] = mapped_column(BIGINT(20), primary_key=True, autoincrement=True, comment='会话ID')
    user_id: Mapped[int] = mapped_column(BIGINT(20), comment='用户ID')
    friend_id: Mapped[int] = mapped_column(BIGINT(20), comment='好友ID')
    last_msg_id: Mapped[Optional[int]] = mapped_column(BIGINT(20), comment='最后一条消息ID')
    last_msg_time: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, comment='最后一条消息时间')
    unread_count: Mapped[int] = mapped_column(INTEGER(11), server_default=text("'0'"), comment='未读消息数')
