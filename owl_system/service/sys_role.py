...
class SysRoleService:
    ...
    def update_role(cls, role:SysRole) -> bool:
        ...
        cls.insert_role_menu(role)  # 插入菜单权限，即使为空列表也正常处理
        return True
    ...
    def insert_role_menu(cls, role: SysRole) -> int:
        ...
        # 调试日志
        logger.debug(f"insert_role_menu - role_id={role.role_id}, menu_ids={role.menu_ids}, type={type(role.menu_ids)}")
        
        if not role.menu_ids:
            logger.debug(f"insert_role_menu - menu_ids 为空或 None，返回 0")
            return 0
        ...
        logger.debug(f"insert_role_menu - 准备插入 {len(menus)} 条菜单权限记录")
        
        if len(menus) > 0:
            result = SysRoleMenuMapper.batch_role_menu(menus)
            logger.debug(f"insert_role_menu - 实际插入了 {result} 条记录")
        else:
            logger.debug(f"insert_role_menu - 没有菜单权限需要插入")
