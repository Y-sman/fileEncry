...
class SysUserService:
    ...
    @classmethod
    def select_user_by_user_name_or_email(cls, account: str) -> Optional[SysUser]:
        """登录用：先按用户名查，若无则按邮箱查"""
        user = SysUserMapper.select_user_by_user_name(account)
        if user:
            return user
        return SysUserMapper.select_user_by_email(account)
    ...
    def check_email_unique(cls, user: SysUser) -> Literal["0","1"]:
        """
        校验邮箱是否唯一（注册时不能与已有用户重复；修改时允许保留自己的邮箱）
        """
    ...
    def check_user_allowed(cls, user: SysUser):
        ...
        """
        # 检查用户是否有超级管理员角色（role_key == "admin"）
        # 只有当用户确实拥有管理员角色时，才禁止操作
        if user.user_id:
            from owl_framework.descriptor.permission import PermissionService
            # 获取用户实际拥有的角色列表
            roles = SysRoleMapper.select_role_permission_by_user_id(user.user_id)
            # 检查是否有 role_key 为 "admin" 的角色
            for role in roles:
                if role and role.role_key == PermissionService.SUPER_ADMIN:
    ...
    def check_user_data_scope(cls, user_id: Optional[int]):
        ...
        # PUT/body 请求可能未经过 QueryValidator，需确保 criterian_meta 存在，避免 mapper 中 AttributeError
        if "criterian_meta" not in g:
            g.criterian_meta = CriterianMeta()
        if not security_util.login_user_is_admin():
            from owl_framework.descriptor.datascope import DataScope
            DataScope(dept=True, user=True).filter_data_scope(security_util.get_login_user().user)
    ...
    def register_user(cls, user: SysUser) -> bool:
        """
        注册用户，默认 dept_id=100、role_id=2（由 RegisterService 传入）

        Args:
            user (SysUser): 用户信息（含 dept_id、role_ids）
        ...
        last_pid = SysUserMapper.insert_user(user)
        if last_pid > 0:
            role_ids = getattr(user, "role_ids", None) or []
            if role_ids:
                cls.insert_user_role(last_pid, role_ids)
        return last_pid > 0
    ...
    def reset_pwd(cls, user: SysUser) -> bool:
        """
        重置用户密码（直接更新 password 列，因 SysUser.password 为 exclude=True 不会进入 model_dump）
        """
        if not (user and getattr(user, "user_id", None)):
            return False
        return SysUserMapper.update_password_by_user_id(user.user_id, user.password) > 0

    @classmethod
    def reset_pwd_by_user_id(cls, user_id: int, encrypted_password: str) -> bool:
        """按用户ID重置密码（加密后的密码）。"""
        if not user_id or not encrypted_password:
            return False
        return SysUserMapper.update_password_by_user_id(user_id, encrypted_password) > 0
    ...
    def import_user(cls, users: List[SysUser],is_update:bool=False) -> str:
        ...
        success_msg = ""
        fail_msg = ""
        default_password = SysConfigService.select_config_by_key("sys.user.initPassword") or "123456"
        default_password = (default_password or "123456").strip()
        ...
        for user in users:
            try:
                ...
                if not dto:
                    enc = security_util.encrypt_password(default_password)
                    user.password = enc.decode("utf-8") if isinstance(enc, bytes) else enc
