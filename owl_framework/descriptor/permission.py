    ...
    def has_perm(cls, permission:str) -> bool:
        ...
        if not login_user: 
            ...
            if not isinstance(login_user, UserMixin):
                return False
            # 检查用户是否是管理员
            if SecurityUtil.is_admin(login_user.user_id):
                return True
            # 所有登录用户都可以访问好友和聊天模块
            if permission.startswith('friend:') or permission.startswith('chat:'):
                return True
...
class PreAuthorize:
    ...
    def __call__(self, func) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            from flask import g
            from owl_common.utils import security_util as SecurityUtil
            g.user = SecurityUtil.get_login_user()
