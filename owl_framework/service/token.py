...
class TokenService:
    ...
    def parse_token(cls, token:str) -> Optional[dict]:
        ...
        Returns:
            dict: token参数claims，如果解析失败则返回None
        '''
        try:
    ...
    def get_login_user(cls, request:Request) -> Optional[LoginUser]:
        ...
        Returns:
            LoginUser: 登录用户信息，如果token无效或用户不存在则返回None
        '''
        ...
        try:
            ...
            logger.warning(f"获取登录用户信息失败: {str(e)}")
            return None
    ...
    @classmethod
    def get_login_user_from_token(cls, token:str) -> Optional[LoginUser]:
        '''
        从token获取登录用户信息
