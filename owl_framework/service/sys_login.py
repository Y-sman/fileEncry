...
class LoginService:
    ...
    def login(cls, login: LoginBody) -> str:
        ...
        sysuser = SysUserService.select_user_by_user_name_or_email(login.username)
        if not sysuser:
...
@lm.request_loader
def load_request(request:Request) -> Optional[LoginUser]:
    """
    加载请求
    ...
    Returns:
        LoginUser: 登录用户，如果token无效则返回None
    """
    try:
        login_user = TokenService.get_login_user(request)
        return login_user
    except Exception as e:
        # 如果token解析失败，返回None，Flask-Login会将其视为未认证用户
        logger = logging.getLogger(__name__)
        logger.warning(f"加载用户请求失败: {str(e)}")
        return None
